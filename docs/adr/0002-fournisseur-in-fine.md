...
normalisée ; puis chaque ligne l'est par article normalisé. La couverture
totale de `cac:Delivery/cbc:ID` est mesurée sur les seules factures UGAP
(40 732, deux formats UBL) ; l'extraction s'applique à tous les fournisseurs,
l'identifiant restant optionnel hors UGAP. Le `cbc:ID` du XML
...
d'article, `item_name` prenant la 1re ligne de `cbc:Description`. Les notes
brutes sont concaténées en silver (`note`, séparateur `\n`) pour
analyse ultérieure ; le libellé « Référence UGAP » n'est plus extrait, sa
...
4 divergences, couverture incluse dans celle de `cac:Delivery` ; les notes
brutes sont conservées dans `note`, sans extraction du libellé. Rapprocher sur
le bon de commande : niveau EJ, +72 lignes mesurées, rattachements croisés
possibles ; il reste conservé en silver pour l'analyse. Rapprocher sur
l'article seul : une référence d'article n'identifie pas la commande. Créer
des lignes gold depuis l'export : le statut `created` n'est jamais atteint et
exposerait les commandes multi-factures au double compte. Dédoublonner
`(commande, article)` : écarterait des paiements partiels réels. Filtrer le
rapprochement sur l'univers « ministères » : les commandes sans facture CPRO
restent suivies en `facture_inconnue`, le périmètre relève du téléchargement.
Renommer les champs `fournisseur_*` en `emetteur_*` :
...
de paiement référencent la ligne gold. Le numéro de facture (`cbc:ID`), la
valeur brute de `cac:Delivery` (JSONB, items multi-identifiants conservés), le
`delivery_id` et les notes brutes sont extraits dans une table silver dédiée
(`silver_cpro_export_facture_xml_facture`) sous les colonnes `numero`,
`delivery`, `delivery_id` et `note` ; `delivery_id` reprend le premier `cbc:ID`
exploitable et reste optionnel, souvent absent hors UGAP. Seul `delivery_id`
sert au gold, les autres restent pour l'analyse. Une table
`silver_cpro_export_facture_xml_facture_status` trace par facture le statut
`Ok`/`Error`. Les exports UGAP doivent être déposés dans `ugap/exports/` pour
alimenter les champs, sinon ils restent NULL. L'enrichissement est recalculé à
chaque run, la table gold étant reconstruite.
